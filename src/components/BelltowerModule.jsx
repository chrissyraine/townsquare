...
export default function BelltowerModule({ business }) {
  ...
  const token = business.tokens?.belltower;

  const fetchState = async () => {
    try {
      setErrorMsg(null);
      const res = await fetch(`${BELLTOWER_API}/api/belfry/${business.slug}/state`, {
      ...
      const bookingsRes = await fetch(`${BELLTOWER_API}/api/belfry/${business.slug}/bookings?from=${today}`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (bookingsRes.ok) {
    ...
    return (
      <div className="animate-fade-in glass-panel" style={{ padding: '40px', textAlign: 'center', maxWidth: '600px' }}>
        ...
        <p style={{ color: 'var(--text-muted)', marginBottom: '24px' }}>
          Please log out and log back in to authenticate with Belltower.
        </p>
      </div>
    );
  }
            ...
            {calendarToken ? (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                <input 
                  ...
                  className="input-field" 
                  readOnly 
                  value={`${BELLTOWER_API}/api/cal/${business.slug}.ics?token=${calendarToken}`}
